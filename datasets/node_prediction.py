...
class KIGraphDatasetSUBGCN(Dataset):
    ...
    def __init__(self, path, mode='train',
        ...
        for i, row in nodes.iterrows():
            ...
            if row['gt'] == 2: # fibroblast and endothelial
                y_nodes[i] = 0 # non-epithelial
            elif row['gt'] == 3:
                y_nodes[i] = 1 # epithelial
            else:
                y_nodes[i] = -1 # Unknown
            x_nodes[i] = row['id']
        y_pos, y_neg, y_unk = np.count_nonzero(y_nodes == 1), np.count_nonzero(y_nodes == 0), np.count_nonzero(y_nodes == -1)
        ...
        if True: #self.mode != 'train':  # Add all edges except positive edges if validation/test
        ...
        print('y_pos, y_neg, y_unk: ', y_pos, y_neg, y_unk)
        x = x_nodes#np.vstack((y_pos, y_neg, y_unk))
        y = y_nodes#np.concatenate((np.ones(y_pos.shape),
                            #np.zeros(y_neg.shape), np.full(y_unk.shape, -1) ))
        ...
        print('Number of epithelial/non-epithelial/unknown datapoints: {}/{}/{}'.format(y_pos, y_neg, y_unk))
